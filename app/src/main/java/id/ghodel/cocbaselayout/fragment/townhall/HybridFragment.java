...
import id.ghodel.cocbaselayout.adapter.townhall.HybridAdapter;
...
public class HybridFragment extends BaseFragment{
    ...
    private void loadData(){
        if (MainUtils.isNetworkConnected(getActivity())) {
            ...
            hybridApiService = retrofitClient.getClient(BuildConfig.BASE_URL).create(APIService.class);
            Call<BaseModel> call = hybridApiService.getCocBase();
            ...
            call.enqueue(new Callback<BaseModel>() {
                ...
                public void onResponse(Call<BaseModel> call, Response<BaseModel> response) {
                    ...
                    if(response.isSuccessful() && response.body() != null){
                        ...
                        hybridAdapter = new HybridAdapter(getActivity(), response.body().getHybrid());
                        recyclerView.setHasFixedSize(true);
                        gridLayoutManager = new GridLayoutManager(getActivity(), 2);
                        recyclerView.setLayoutManager(gridLayoutManager);
                        recyclerView.setItemAnimator(new DefaultItemAnimator());
                        recyclerView.setAdapter(hybridAdapter);
    ...
    public void onCreateOptionsMenu(@NonNull Menu menu, @NonNull MenuInflater inflater) {
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String newText) {
                if(newText != null){
                    hybridAdapter.getFilter().filter(newText);
                }
                return false;
            }
        });
